<script>
    ...
    const login = async () => {
      try {
        const response = await fetch('https://car-viewer-ochre.vercel.app/dealerpasswordchange', {
          method: 'POST',
          headers: {
  ...
  <main class="flex flex-col items-center h-full gap-4 bg-slate-300 p-4 rounded-xl">
    <h2 class="text-4xl text-white font-bold mb-4">Dealer Password Reset</h2>
  ...
  <div class="flex justify-center">
      <button class="btn mt-2" type="submit">Change Password</button></div>
    </form>
