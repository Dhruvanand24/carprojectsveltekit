...
<main class="flex flex-col items-center h-full gap-4 p-4 bg-red-400 rounded-xl">
  ...
  <form class="flex flex-col" on:submit|preventDefault={handleSignup}>
    ...
    <input type="text" placeholder="enter location" id="user_location" class="input bg-white input-bordered w-full max-w-xs" bind:value={user_location} required />
  ...
  <p class="text-white">
    Already have an account? <span on:click={() => dispatch("flip", "user")} class="text-black font-bold cursor-pointer select-none">Log in</span>
  </p>
</main>
