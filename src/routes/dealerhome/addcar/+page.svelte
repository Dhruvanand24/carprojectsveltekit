  ...
  const handleSubmit = async (event) => {
    ...
    try {
      const response = await fetch(
        "https://car-viewer-ochre.vercel.app/addcars",
        {
          method: "POST",
          headers: {
          ...
          body: JSON.stringify({
            ...
            name,
            model,
            car_info: { imageurl, dealership_id: id }, 
            dealership_id: id,
          }),
        }
      );
...
<main class="min-h-screen flex items-center justify-center w-full p-8">
  ...
  >
    <label for="name" class="block mb-2 text-sm font-semibold text-gray-600"
      >Name:</label
    >
    <input
      type="text"
    ...
    <label
      for="model"
      class="block mt-4 mb-2 text-sm font-semibold text-gray-600">Model:</label
    >
    <input
      type="text"
      id="model"
    ...
    <label
      for="type"
      class="block mt-4 mb-2 text-sm font-semibold text-gray-600">Type:</label
    >
    <input
      type="text"
      id="type"
    ...
    <label
      for="image"
      class="block mt-4 mb-2 text-sm font-semibold text-gray-600">Image:</label
    >
    <input
      type="text"
      id="image"
